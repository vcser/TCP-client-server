...
void send_encrypted_file(struct crypto_context *context, int sock,
    ...
    unsigned char buf_out[STEP + crypto_secretstream_xchacha20poly1305_ABYTES],
        tag;
    // unsigned long long out_len;
    int n = min(file_size, STEP), max_size = file_size + n, len = -n;
    if (crypto_secretstream_xchacha20poly1305_init_push(
            &context->state, context->header, context->k) == -1) {
    ...
    do {
        ...
        if (crypto_secretstream_xchacha20poly1305_push(
                &context->state, buf_out, NULL, buf_in, min(STEP, max_size),
                NULL, 0, tag) == -1) {
            fprintf(stderr, "Error: could not push buffer\n");
            exit(EXIT_FAILURE);
        }
    } while ((n = write(sock, buf_out,
                        min(STEP, max_size) +
...
void recv_encrypted_file(struct crypto_context *context, int sock,
    ...
    while ((n = read(sock, buf_in,
                     min(STEP, max_size) +
                         crypto_secretstream_xchacha20poly1305_ABYTES)) > 0) {
        if (crypto_secretstream_xchacha20poly1305_pull(
                &context->state, buf_out, &n, &tag, buf_in, n, NULL, 0) == -1) {
            printf("n: %lld\n", n);
